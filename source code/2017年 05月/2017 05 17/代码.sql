...
on primary
( name='pxscj_data',
  filename='E:\SQLserver\2017 05 17\pxscj.mdf',
  size=3mb,
  maxsize=100mb,
  filegrowth=1mb
)
log on
( name='pxscj_log',
  filename='E:\SQLserver\2017 05 17\pxscj.ldf',
  size=1mb,
  maxsize=10mb,
  filegrowth=10%
)
...
--1、从三个表的连接中返回指定学生的学号、姓名、所选课程名称及该课程的成绩。
--   该存储过程在参数中使用了模式匹配，如果没有提供参数，则使用预设的默认值。

create proc xsdcj @xs varchar(2) = '王' as
select xsb.学号,xsb.姓名,KCB.课程名,CJB.成绩 
from xsb join CJB join KCB on KCB.课程号=CJB.课程号 on CJB.学号=xsb.学号
where 姓名 like @xs+'%'
...
alter proc xsdcj @xs varchar(2) = '李' as			--其实就是create换成了alter而已
select xsb.学号,xsb.姓名,KCB.课程名,CJB.成绩 
from xsb join CJB join KCB on KCB.课程号=CJB.课程号 on CJB.学号=xsb.学号
where 姓名 like @xs+'%'
go
